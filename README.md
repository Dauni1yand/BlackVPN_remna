...
Чтобы снести всё и поставить заново с чистого листа (свежий Postgres,
свежие секреты, заново спросит API-токен и id админов) — например, пока
только настраиваешь всё и хочется начать без накопившегося мусора после
неудачных попыток:
...
Перед сносом попросит набрать `RESET` для подтверждения (это необратимо —
удаляет всех пользователей, ноды, конфиги из базы панели). Кеш SSL-
сертификатов Caddy не трогает, чтобы не упереться в лимиты Let's Encrypt
при частых пересозданиях.

Обычный (не `RESET_INSTALL`) запуск идемпотентен — повторные шаги
пропускаются, если уже выполнены — и
делает следующее:
